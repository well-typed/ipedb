...
import Control.Exception (SomeException (..), bracketOnError, bracket_, catch, throwIO)
...
-- instance (Coercible v u, LSMT.SerialiseKey u) => Key (SerialiseVia v u)
...
instance (Coercible v u, LSMT.SerialiseValue u) => Value (SerialiseVia v u)
...
-- Internal helper.
--
-- Compute an `LSMT.SnapshotName` from a `FilePath`.
-- -}
-- toSnapshotName :: FilePath -> LSMT.SnapshotName
-- toSnapshotName filePath =
--   LSMT.toSnapshotName . map underscoreInvalidChar . SF.takeBaseName $ filePath
--  where
--   underscoreInvalidChar :: Char -> Char
--   underscoreInvalidChar c =
--     if c `elem` ['a' .. 'z'] || c `elem` ['0' .. '9'] || c == '-' || c == '_' then c else '_'
...
snapshotLabel = LSMT.SnapshotLabel (T.pack "ipedb:IpeDB.Database")
