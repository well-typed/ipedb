...
import IpeDB.Database.HashMap qualified as HashMap
import IpeDB.Database.LSMTree qualified as LSMTree

data HashMap

instance Database HashMap where
  type Key HashMap k = HashMap.Key k
  type Value HashMap v = HashMap.Value v
  ...
  insert :: (Key HashMap k) => Table HashMap k v -> k -> v -> IO ()
  insert = HashMap.insert

  lookup :: (Key HashMap k) => Table HashMap k v -> k -> IO (Maybe v)
  lookup = HashMap.lookup

  type IteratorOptions HashMap = HashMap.IteratorOptions

  withIterator :: IteratorOptions HashMap -> Table HashMap k v -> (SourceT IO (k, v) -> IO a) -> IO a
  withIterator = HashMap.withIterator

  type IndexerOptions HashMap = HashMap.IndexerOptions

  indexer :: (Key HashMap k) => (e -> Maybe (k, v)) -> IndexerOptions HashMap -> Table HashMap k v -> ProcessT IO e Void
  indexer = HashMap.indexer

data LSMTree

instance Database LSMTree where
  type Key LSMTree k = LSMTree.Key k
  type Value LSMTree v = LSMTree.Value v
